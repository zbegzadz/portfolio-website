...
function maakProjectFilter() {
    const kaarten = document.querySelectorAll<HTMLElement>('.project-card');
    if (!kaarten.length) return;

    const projecten: Project[] = Array.from(kaarten).map(kaart => ({
        tags: Array.from(kaart.querySelectorAll<HTMLElement>('.tag'))
    ...
    const alleTags: string[] = [...new Set(projecten.flatMap(p => p.tags))];

    const container = document.querySelector<HTMLElement>('.projects-inner');
    if (!container) return;
    ...
    const knoppen: HTMLButtonElement[] = [];

    function maakKnop(tekst: string, waarde: string) {
